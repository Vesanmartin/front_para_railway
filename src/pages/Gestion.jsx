...
function Gestion() {
  ...
  useEffect(() => {
    const cargar = async () => {
      try {
        const respuesta = await fetch(`${GATEWAY_URL}/api/sucursales`);
  ...
  const crearSucursal = async () => {
    try {
      await fetch(`${GATEWAY_URL}/api/gestion/gestion`, {
        method: "POST",
  ...
  const eliminarSucursal = async (id) => {
    try {
      await fetch(`${GATEWAY_URL}/api/gestion/gestion/${id}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` }
      });
  ...
  const eliminarUsuario = async (id) => {
    try {
      await fetch(`${GATEWAY_URL}/api/auth/usuarios/${id}`, {
        method: "DELETE",
        headers: { "Authorization": `Bearer ${token}` }
      });
  ...
  // Las funciones Lambda apuntan a serverless-offline (no disponible en Railway)
  // Se mantienen con PLACEHOLDER para dejar claro que son funciones no implementadas
